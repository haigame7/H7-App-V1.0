...
render(){
  ...
  let btn;
    if (this.state.fightData.CertifyState == '0') {
      ...
      btn =
      (
        <TouchableHighlight style={this.state.loading ? [styles.btn, styles.btndisable] : styles.btn} underlayColor={'#000000'}>
          <Text style={styles.btnfont} >{this.state.btn_msg}</Text>
        </TouchableHighlight>
      )
    } else {
  ...
  return (
    <View >
      ...
      <View key={'dota2id'} style={styles.logininput}>
        <TextInput {...fields[0]} defaultValue={this.state.fightData.GameID || ''} underlineColorAndroid = 'transparent' onChangeText={(text) => this.state.data.dota2id = text} />
      </View>
      ...
      <View key={'certifyid'} style={styles.logininput}>
        <TextInput {...fields[1]} defaultValue={this.state.fightData.CertifyName}/>
